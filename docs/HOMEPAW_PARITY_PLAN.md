...
| 23 | Coverage detection | Implemented foundation | Booking wizard computes available-vs-reserved from tenant-scoped package/reservation queries; Customer 360 uses app.list_customer_package_coverage. Over-allocation warning and locked, per-pet server validation exist. |
| 24 | Packages and memberships | Partial+ | Recurrence/per-pet columns, source invoice, per-pet sale selection, and manual ledger-backed renewal exist. Catalog term configuration UI and a billable renewal invoice/payment flow are still missing; current renewal grants sessions without a new invoice. |
| 25 | Membership administration | Partial+ | /programs/memberships has status/urgency filters, cached/reserved balance, guarded renew/archive, and Customer 360 links. Purchased-term editing and a complete ledger-derived history are still missing. |
| 26 | Subscription reconciliation | Partial+ | Read-only balance/reservation comparison and authorized, revision-guarded cache repair exist. Reconciliation does not yet verify source/renewal invoices or all ledger lifecycle invariants; full gate 7/8 replay remains open. |
