...
| 24 | Packages and memberships | Done | Recurrence/per-pet/session/price/validity/rollover terms are configurable in /programs (RLS-guarded table writes, membership.manage, never rewrites already-sold entitlements). Per-pet sale/allocation works end to end. Manual renewal now issues a real invoice (order + invoice + invoice_lines + ledger row, billing_mode=package_sale, status=issued -- never auto-marked paid) with an explicit, staff-confirmed branch; sessions roll over and expiry extends from greatest(now, current expiry) uniformly across active/exhausted/expired/legacy packages. Shipped in 20260926090000_membership_renewal_billing_and_reconciliation.sql. |
| 25 | Membership administration | Done | /programs/memberships: status/urgency filters (urgency now also reacts to an exhausted balance, not just expiry), guarded renew (issues an invoice)/archive/reactivate, per-membership ledger + reservation history (purchases/renewals/consumption/adjustments with their invoice link, when linked), legacy (no source invoice) flag, and Customer 360 links. |
| 26 | Subscription reconciliation | Done | Read-only preview verifies actual reservation<->ledger LINKS (consumption_ledger_id/reversal_ledger_id), not just matching counts; flags orphan consumption ledger rows, unlinked purchase/renewal invoices, over-reservation, expiry/status inconsistency, and legacy (missing source invoice) provenance as manual_review_issues, separate from auto-repairable cache-vs-ledger drift. Guarded repair (membership.manage, revision-checked, idempotent, audited) only ever touches the auto-repairable case. GATE 7 and GATE 8 now genuinely re-ran as literally scripted (via docker exec inside the disposable PostgreSQL 16 container, resolving the host/WSL2 TCP limitation) and passed. |
