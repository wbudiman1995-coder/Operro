...
| 17 | Groomer performance | Implemented | Monthly cycle view combines attributable completed pets and commission with repeat-customer retention, timeline-derived actual duration, before/after documentation rate and linked complaint count; no invoice revenue is falsely split across multi-groomer bookings |
...
| 19 | Complaints | Implemented | Tenant/branch-isolated complaint register links customer, booking, pet and groomer; includes severity/category filters, guarded lifecycle transitions, resolution and recovery notes, immutable timeline plus generic audit history, and authorized CSV export |
...
| 23 | Coverage detection | Done | Booking wizard computes available-vs-reserved from tenant-scoped package/reservation queries and counts every selected service against the same package, including two services on the same pet (fixed a real under-count); Customer 360 uses app.list_customer_package_coverage. Per-pet eligibility is enforced server-side under lock; the final session of a race is proven single-winner by a real two-connection test. |
| 24 | Packages and memberships | Done | Recurrence/per-pet/session/price/validity/rollover terms are configurable in /programs (RLS-guarded table writes, membership.manage, never rewrites already-sold entitlements). Per-pet sale/allocation works end to end. Manual renewal issues a real invoice (order + invoice + invoice_lines + ledger row, billing_mode=package_sale, status=issued -- never auto-marked paid) bound to a server-validated preview (terms_fingerprint, refused if the catalog changed since the preview was generated) with an explicit, staff-confirmed branch; rollover_policy actually governs the top-up (protects held sessions, discards only unreserved excess when 'none'); catalog per-pet/service drift that would retarget an existing entitlement is refused, not silently applied. Shipped in 20260926090000_membership_renewal_billing_and_reconciliation.sql. |
| 25 | Membership administration | Done | /programs/memberships: status/urgency filters (urgency also reacts to an exhausted balance, not just expiry), guarded renew (issues a preview-bound invoice)/archive/reactivate, per-membership ledger + reservation history, legacy (no source invoice) flag, Customer 360 links, AND guarded purchased-term editing (app.update_customer_package_terms: expires_at always correctable; pet/service correctable only before the entitlement has ever been reserved/consumed/renewed; revision-checked, reasoned, audited in timeline_events; never a balance/invoice edit). |
| 26 | Subscription reconciliation | Done | Read-only preview verifies actual reservation<->ledger LINKS tied to the SPECIFIC purchase/renewal event (request_key + renewal_of, not just customer+product), not just matching counts; validates the HISTORICAL source_invoice_id the same way (previously checked only for NULL); flags orphan consumption ledger rows, unlinked/invalid renewal invoices, invalid source invoices, over-reservation, expiry/status inconsistency, and legacy provenance as manual_review_issues, separate from auto-repairable cache-vs-ledger drift. Guarded repair (membership.manage, revision-checked, idempotent, audited) only ever touches the auto-repairable case. GATE 7 and GATE 8 re-ran as literally scripted (via docker exec inside the disposable PostgreSQL 16 container) and passed; the migration-upgrade replay is strengthened to a byte-identical content-hash comparison, not row count. |
