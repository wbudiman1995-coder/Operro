...
test("one link can create only one pending submission and is consumed",()=>{assert.match(sql,/uq_onboarding_submission_link unique \(link_id\)/);assert.match(sql,/where token_hash=.*for update/s);assert.match(sql,/set status='submitted'/)});
