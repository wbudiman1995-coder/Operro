...
import { PackageSaleForm } from "@/components/pilot-forms";
...
export default async function ProgramsPage() {
  ...
  const activePackages = catalog.packages.filter((item) => item.active);
  return <WorkspaceShell {...workspace} activePath="/programs"><PageHeader eyebrow="Retensi pelanggan" title="Paket grooming" description="Pantau produk paket dan sisa sesi pelanggan tanpa mencampur saldo dengan pembayaran." />
    <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="font-bold">Jual paket</h2><p className="mb-5 mt-1 text-xs text-slate-500">Paket tidak bisa jadi item invoice (bukan katalog layanan/produk) — pembayarannya dicatat langsung di sini.</p><PackageSaleForm customers={customers.customers.map((c) => ({ id: c.id, name: c.name }))} packages={activePackages} branches={catalog.branches} /></section>
