...
export function LogoutButton({ compact = false, inverse = false }: { compact?: boolean; inverse?: boolean }) {
  ...
  return (
    <div className={compact ? "w-full" : "inline-flex flex-col items-end"}>
      <button
        ...
        className={
          inverse
            ? "flex h-11 w-full items-center justify-center rounded-[10px] border border-white/10 bg-white/[0.06] px-4 text-sm font-semibold text-slate-200 transition hover:border-white/20 hover:bg-white/10 hover:text-white focus:outline-none focus:ring-4 focus:ring-white/10 disabled:opacity-50"
            : compact
