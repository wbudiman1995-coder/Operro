...
interface Props { organizations: readonly AccessibleOrganization[]; activeOrganization: AccessibleOrganization; userEmail: string; data: BookingWorkspaceData; weekStartISO: string; defaultTimezone: string; created: boolean; preselectedCustomerId?: string }
...
export function BookingShell({ organizations, activeOrganization, userEmail, data, weekStartISO, defaultTimezone, created, preselectedCustomerId }: Props) {
  ...
  return <WorkspaceShell organizations={organizations} activeOrganization={activeOrganization} userEmail={userEmail} activePath="/bookings">
    ...
    <section id="booking-baru" className="mt-8 scroll-mt-24"><BookingWizard branches={data.branches} customers={data.customers} pets={data.pets} services={data.services} resources={data.resources} defaultStart={`${weekStartISO}T09:00`} preselectedCustomerId={preselectedCustomerId} /></section>
  </WorkspaceShell>;
}
